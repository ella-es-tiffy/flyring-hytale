...
public class FlyRing {
    ...
    private void setup() {
        ...
        // Heartbeat - jede Sekunde Flight sync
     ...
     * state
     * (e.g., after dismounting from a mount that removed the flying state)
     */
    ...
    public void updateFlightStatus(Player player) {
        if (player == null)
            return;

        boolean hasRing = RingUtils.hasRing(player, FLY_RING_ITEM_ID);
        setForceFlightState(player, hasRing);
    }
